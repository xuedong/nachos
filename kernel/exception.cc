...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
...
#ifdef ETUDIANTS_TP
    case SC_SEM_CREATE: {
        DEBUG('e', (char*)"Debug: Semaphore creation.\n");

        int name_size;
        int32_t name_addr;
        name_addr = g_machine->ReadIntRegister(4);
        name_size = GetLengthParam(name_addr);
        char name[name_size];
        GetStringParam(name_addr, name, name_size);

        int32_t value = g_machine->ReadIntRegister(5);

        Semaphore *sem = new Semaphore(name, value);
        int32_t tid;
        tid = g_object_ids->AddObject(sem);
    ...
    case SC_SEM_DESTROY: {
        DEBUG('e', (char*)"Debug: Semaphore destruction.\n");

        int32_t tid = g_machine->ReadIntRegister(4);
        Semaphore *sem = (Semaphore *)g_object_ids->SearchObject(tid);

        if(sem && sem->typeId == SEMAPHORE_TYPE_ID){
            /* A problem may occurs if some thread is still locked by the
             * semaphore. A solution would be to look into the semaphore, but
             * that would break the semaphore's secret.
             */
            delete sem;
            g_machine->WriteIntRegister(2, 0);
        }else{
            g_syscall_error->SetMsg((char*)"Invalid semaphore.", InvalidSemaphoreId);
            g_machine->WriteIntRegister(2, -1);
        }
    ...
    case SC_P: {
        DEBUG('e', (char*)"Debug: Semaphore P.\n");
    ...
    case SC_V: {
        DEBUG('e', (char*)"Debug: Semaphore V.\n");
    ...
    case SC_LOCK_CREATE: {
        DEBUG('e', (char*)"Debug: Lock creation.\n");
        ...
        int32_t tid;
        tid = g_object_ids->AddObject(lock);
    ...
    case SC_LOCK_DESTROY: {
        DEBUG('e', (char*)"Debug: Lock destruction.\n");

        int32_t tid = g_machine->ReadIntRegister(4);
        Lock *lock = (Lock *)g_object_ids->SearchObject(tid);

        if(lock && lock->typeId == LOCK_TYPE_ID){
            /* Same as semaphores. */
            delete lock;
            g_machine->WriteIntRegister(2, 0);
        }else{
            g_syscall_error->SetMsg((char*)"Invalid lock.", InvalidLockId);
            g_machine->WriteIntRegister(2, -1);
        }
    ...
    case SC_LOCK_ACQUIRE: {
        DEBUG('e', (char*)"Debug: Lock acquire.\n");
    ...
    case SC_LOCK_RELEASE: {
        DEBUG('e', (char*)"Debug: Lock release.\n");
    ...
    case SC_COND_CREATE: {
        DEBUG('e', (char*)"Debug: Conditionnal lock creation.\n");
        ...
        int32_t tid;
        tid = g_object_ids->AddObject(cond);
    ...
    case SC_COND_DESTROY: {
        DEBUG('e', (char*)"Debug: Conditionnal lock destruction.\n");

        int32_t tid = g_machine->ReadIntRegister(4);
        Condition *cond = (Condition *)g_object_ids->SearchObject(tid);

        if(cond && cond->typeId == CONDITION_TYPE_ID){
            /* Same as semaphores. */
            delete cond;
            g_machine->WriteIntRegister(2, 0);
        }else{
            g_syscall_error->SetMsg((char*)"Invalid conditional variable.", InvalidConditionId);
            g_machine->WriteIntRegister(2, -1);
        }
    ...
    case SC_COND_WAIT: {
        DEBUG('e', (char*)"Debug: Conditionnal lock wait.\n");
    ...
    case SC_COND_SIGNAL: {
        DEBUG('e', (char*)"Debug: Conditionnal lock signal.\n");
    ...
    case SC_COND_BROADCAST: {
        DEBUG('e', (char*)"Debug: Conditionnal lock broadcast.\n");
