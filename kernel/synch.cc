...
Semaphore::V() {
    ...
    IntStatus old = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);
    ++value;
    Thread *first = (Thread*)queue->Remove();
    if(first != NULL){
        g_scheduler->ReadyToRun(first);
    }
    g_machine->interrupt->SetStatus(old);
#endif
}
...
void Lock::Release() {
    ...
    IntStatus old = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);
    if(!free){
        Thread *first = (Thread*)sleepqueue->Remove();
        if(first != NULL){
            g_scheduler->ReadyToRun(first);
        }else{ // sleepqueue is empty
            free = true;
        }
    }
    g_machine->interrupt->SetStatus(old);
...
#ifdef ETUDIANTS_TP
    IntStatus old = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);
    Thread *first = (Thread*)waitqueue->Remove();
    if(first != NULL){
        g_scheduler->ReadyToRun(first);
    }
    g_machine->interrupt->SetStatus(old);
...
void Condition::Broadcast() { 
    ...
    IntStatus old = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);
    Thread *first = NULL;
    while((first = (Thread*)waitqueue->Remove())){
        g_scheduler->ReadyToRun(first);
    }
    g_machine->interrupt->SetStatus(old);
#endif
