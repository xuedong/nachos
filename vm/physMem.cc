...
*/
//-----------------------------------------------------------------
int PhysicalMemManager::AddPhysicalToVirtualMapping(AddrSpace* owner,int vp)
{
#ifndef ETUDIANTS_TP
  ...
  owner->translationTable->setPhysicalPage(vp, pp);
  owner->translationTable->setBitValid(vp);
  owner->translationTable->clearBitM(vp);
  tpr[pp].locked = true;
  tpr[pp].virtualPage = vp;
  tpr[pp].owner = owner;
