...
void matrix_mult(matrix_t *a, matrix_t *b, matrix_t *r, size_t nthread){
    ...
    ThreadId t[nthread]; // On alloue de l'espace pour les ids de nos threads.
    struct matrix_mult_proc_t proc[nthread];
    size_t i;
    for(i = 0; i < nthread; ++i){ // Pour chaque thread.
		...
		// On lance le calcul.
		t[i] = newThread("matrix-thread", (int)(void *(*)(void*))matrix_mult_thread, (int)(void*)&proc[i]);
    }

    for(i = 0; i < nthread; ++i){
        Join(t[i]); // On attend la fin de tous les threads.
    }
}
...
int main(int argc, char *argv[]){
    ...
    //    n = atoi(argv[1]); // On change la taille des matrices.
        ...
        if(argc > 1){
            nthread = n_atoi(argv[1]); // On change le nombre de threads.
        }
    //}
    ...
    if(matrix_compare(&r1, &r2)){
        ...
        n_puts("Oh my... ! This is not working at all!"); // :(
    }
